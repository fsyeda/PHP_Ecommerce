...
class Show extends CI_Model {
	...
	public function display_product($id)
	{
		...
		$shopping_cart_details = $this->db->query($query, $value)->row_array();

		$user = $this->session->userdata('user_session'); // Set the logged in user's information in session.
		$userid = intval($user['id']);
		$query1 = "SELECT COUNT(product_id) FROM carts where user_id = ?";
		$value1 = $userid;
		$shopping_cart_details['count'] = $this->db->query($query1, $value1)->row_array();
		// var_dump($shopping_cart_details['count']['COUNT(product_id)']); die();
...
// Uncomment this
	...
	public function update_cart($product)
	{
		...
		$values = array('user_id' => $userid, 'product_id' => $product['id'], 'quantity' => $product['quantity']);
				// var_dump($values);die();
...
// Uncomment this
	public function shopping_cart()
	{
		...
		$query = "SELECT products.name, carts.quantity, products.description, products.quantity_sold, products.price, products.id, products.category_id from products 
